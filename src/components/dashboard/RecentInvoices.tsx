...
import type { DashboardInvoice } from "@/lib/types";
...
export function RecentInvoices() {
  ...
  useEffect(() => {
    fetch("/api/records?type=invoices")
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                  <td className="px-3 py-2 text-zinc-500 font-mono">{inv.reference_code}</td>
                  <td className="px-3 py-2 text-zinc-500 font-mono">{inv.bill_number ?? "—"}</td>
                  ...
                  <td className="px-3 py-2 text-right text-zinc-600 dark:text-zinc-400">
                    {inv.total != null ? `$${Number(inv.total).toLocaleString("es-CO")}` : "—"}
                  </td>
                  <td className="px-3 py-2 text-zinc-400">{new Date(inv.created_at).toLocaleDateString()}</td>
                  <td className="px-3 py-2">
                    <DownloadPdfButton
                      type="invoice"
                      number={inv.bill_number ?? ""}
                      disabled={!inv.bill_number || inv.status !== "ACCEPTED"}
                    />
                  </td>
                </tr>
              ))}
